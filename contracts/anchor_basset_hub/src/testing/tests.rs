...
use crate::msg::QueryMsg::{
    AllHistory, Config, CurrentBatch, ExchangeRate, Parameters as Params, State, UnbondBatches,
    UnbondRequests, WhitelistedValidators, WithdrawableUnbonded,
};
...
/// parameters' storage, config storage stores the creator,
...
/// Covers if delegate message is send to the specified validator,
...
/// Covers if the receive message is send by token contract,
...
/// Covers if the epoch period is passed, undelegate message is send,
/// state storage is updated to the new changed value,
/// current epoch is updated to the new values,
/// request is stored in unbond wait list, and unbond history map is updated
#[test]
...
/// Covers if the pick validator sends different undelegate messages
/// to different validators, when a validator does not have enough delgation.
#[test]
...
/// update exchange rate after and before slashing.
#[test]
pub fn proper_slashing() {
    ...
    let query_exchange_rate: ExchangeRateResponse =
        from_binary(&query(&deps, ex_rate).unwrap()).unwrap();
    assert_eq!(query_exchange_rate.rate.to_string(), "0.900047370914258645");
...
/// Covers if withdraw rate is updated before and after withdraw_unbonded call,
/// the finished amount is accurate, user requests are removed from the wait list, and
...
/// Covers slashing during unbonded period and its effect on the finished amount.
#[test]
...
/// Covers withdraw_unbonded/inactivity in the system while there are slashing event.
#[test]
...
pub fn proper_recovery_fee() {
    ...
    let query_exchange_rate: ExchangeRateResponse =
        from_binary(&query(&deps, ex_rate).unwrap()).unwrap();
    let new_exchange = query_exchange_rate.rate;
